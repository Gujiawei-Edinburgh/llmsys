...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        return multiplied + self.bias.value
        # END ASSIGN1_2
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        averaged = embeddings.mean(1).view(batch_size, self.embedding_dim)
        out = self.l1.forward(averaged)
        out = out.relu()
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for batch_num, example_num in enumerate(
                ...
                train_predictions += get_predictions_array(y_train_tensor, out)
                total_loss += loss[0]
                n_batches += 1
            ...
            if data_val is not None:
                ...
                backend = minitorch.TensorBackend(SimpleOps)
                x_val_tensor = minitorch.tensor(
                    ls=X_val,
                    backend=backend,
                    requires_grad=False
                )
                y_val_tensor = minitorch.tensor(
                    ls=y_val,
                    backend=backend,
                    requires_grad=False
                )
                out = model.forward(x_val_tensor)
                validation_predictions = get_predictions_array(y_val_tensor, out)
