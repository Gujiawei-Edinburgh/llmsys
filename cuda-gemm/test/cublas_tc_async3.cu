...
void coarse_2d_tc_async3(float *a, float *b, float *c, int C_rows, int C_cols, int A_cols);
...
int main()
{
    ...
    for (int size : sizes) {
        ...
        coarse_2d_tc_async3(d_a.ptr, d_b.ptr, d_c_naive.ptr, rows, cols, kdim);
        ...
        std::printf("N=%d coarse_2d_tc_async3=%.3f ms (%.3f TFLOP/s) cuBLAS=%.3f ms (%.3f TFLOP/s) max_abs_err=%.6f\n",
                    size,
                    naive_ms,
                    naive_tflops,
                    cublas_ms,
