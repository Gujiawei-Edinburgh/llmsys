...
int main()
{
    ...
    for (int size : sizes) {
        ...
        coarse_2d_vec(d_a.ptr, d_b.ptr, d_c_naive.ptr, rows, cols, kdim);
        // If the kernel faults (e.g., illegal access), cuBLAS may fail later with
        // CUBLAS_STATUS_EXECUTION_FAILED. Catch it at the source.
        ...
        std::printf("N=%d coarse_2d_vec=%.3f ms (%.3f TFLOP/s) cuBLAS=%.3f ms (%.3f TFLOP/s) max_abs_err=%.6f\n",
                    size,
                    naive_ms,
                    naive_tflops,
                    cublas_ms,
